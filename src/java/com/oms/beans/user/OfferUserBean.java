 ...
 * @author ersan
 */
@ManagedBean(name="offerUserBean")
@SessionScoped
public class OfferUserBean {
      ...
      try 
        {   
            ...
            teklifSatirlari = offerUserDao.getirTeklifSatirlari(teklifBaslik.getRECORD_ID(),
                                                                teklifBaslik.getERP_FIRMA_NUMBER());
            disabled = true;
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Teklif yanıtınız başarıyla gönderilmiştir. Yanıt verdiğiniz için teşekkür ederiz.", ""));
        } 
        catch (Exception ex) 
        {
    ...
    {
        try {
          offerUserDao.download(teklifBaslik.getRECORD_ID(),
